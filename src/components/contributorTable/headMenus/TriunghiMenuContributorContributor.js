...
export default function TriunghiMenuContributorContributor({ handleMenuFilter }) {
    ...
    const handleClick = (event) => {
        client.get('tab_contributors/filter/contributor').then((contributor_data) => {
            setState({
                loading: false,
    ...
    const handleFilterByName = (event) => {
        if (event.target.value) {
            client.get(`tab_contributors/filter/contributor?search=${event.target.value}`).then((contributor_data) => {
                setState({
                    loading: false,
                    contributor_data: contributor_data,
                });
